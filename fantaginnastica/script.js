...
function create_athlete_table() {
...

	{
		...
		td0.appendChild(document.createTextNode("nazione"));
		td1.appendChild(document.createTextNode("nome"));
		td2.appendChild(document.createTextNode("prezzo"));
		td3.appendChild(document.createTextNode("tipo"));
		td4.appendChild(document.createTextNode("scegli"));
	...
	for (let i = 0; i < ath.length; i++) {
		...
		if(ath[i].category == 0){
			td3.appendChild(document.createTextNode("in squadra"));
		} else {
			td3.appendChild(document.createTextNode("individualista"));
		}
	...
	document.getElementById('athletes').appendChild(tbl);
}
...
function create_scoreboard(){
...

	{
		...
		td0.appendChild(document.createTextNode("punti"));
		td1.appendChild(document.createTextNode("nome"));
	...
	for (let i = 0; i < ss.length; i++) {
		...
		if(check_team(team)[0]) {
			console.log("team "+i+" by "+ss[i].name+" " + ss[i].id + " is no good");
			continue;
		}
